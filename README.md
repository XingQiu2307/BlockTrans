...
- 🎯 **智能识别** - 自动定位 `*/text/*.lang`、`*/texts/*.lang` 等语言文件（常见于包含 `res` 的路径）
...
2. 系统自动解压并智能识别 `*/text/*.lang`、`*/texts/*.lang` 语言文件（常见于包含 `res` 的路径）
3. AI 翻译后显示结果，支持在线编辑修改
4. 确认翻译后自动重命名为 zh_CN.lang 并重新打包
5. 下载完整的翻译版附加包，**保留原有文件结构和格式**
